...
def getPrintZValue(lineBlock):
	...
	for line in lineBlock:
		if line.startswith(';TYPE:CUSTOM PAUSE START'):
			ispauseskipit=1
		if line.startswith(';TYPE:CUSTOM PAUSE END'):
			ispauseskipit=0
		if ispauseskipit == 0:
	...
	for lIndex in xrange(len(lines)):
		...
		if pauseState == 2:
			...
			if g == 1 or g == 0:# We will do the pause just before printing content. We need to pause from the previous XY position. Not the current.
				...
				pauseState = 0
				f.write(";TYPE:CUSTOM PAUSE START\n")
				#Retract
				f.write("M83\n")
